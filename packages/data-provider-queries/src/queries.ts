...
const LUKSO_PROXY_API = "https://api.universalprofile.cloud";

/**
 * Options for query promise for Call
 */
...
/**
 * Options for query promise for Data
 */
...
// Allow 80 requests per second (the Twitter search limit). Also understands
// 'second', 'minute', 'day', or a number of milliseconds
const limiter = new RateLimiter({ tokensPerInterval: 80, interval: "second" });

async function convert<T = unknown>(
...
export type VerifiableURI = {
	verification: {
		method: HexString | "keccak256(utf8)" | "keccak256(bytes)";
		data: HexString;
	};
	url: string;
};
