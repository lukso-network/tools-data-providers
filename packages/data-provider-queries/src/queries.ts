...
import { type AbiItem, toNumber } from "web3-utils";
...
import { type AbiInput, type AbiFunctionFragment } from "web3-types";

import LSP2FetcherWithMulticall3Contract from "../contracts/LSP2FetcherWithMulticall3";
...
const NOT_SUCCESSFUL_NULL = "not successful (assume null)";
...
const networkInfo: {
  chainId?: string;
  fetcherContractAddress?: `0x${string}`;
  web3?: Web3;
} = {};

export function setNetworkConfig({
  chainId,
  fetcherContractAddress,
  rpc,
}: {
  chainId: string;
  fetcherContractAddress: `0x${string}`;
...
async function doQueries() {
  ...
  running++;
  try {
    const { chainId, fetcherContractAddress, web3 } = networkInfo;
    ...
    assert(web3?.eth.Contract, "No contract constructor");
    const lsp2CustomContract = new web3.eth.Contract(
      LSP2FetcherWithMulticall3Contract.abi as readonly AbiItem[],
      fetcherContractAddress
    );
    ...
    try {
      for (const [
        ...
        if (getDataForTokenId) {
          ...
          for (const tokenQueries of Object.values(tokenIds)) {
            ...
            assert(abi?.name, METHOD_NOT_FOUND);
            if (abi) {
              const call = ABICoder.encodeFunctionCall(abi, [
              ...
              multicall.push({
                ...
                selector(data: string) {
                  ...
                  const result = ABICoder.decodeParameters(
                    (abi?.outputs || []) as unknown as AbiInput[],
                    data
                  )[0];
        ...
        if (call) {
          for (const query of call) {
            if (address === fetcherContractAddress) {
              const abi = defaultAbi.find((item) => {
                if (item.type !== "function") {
                  return false;
                }
                const { name, inputs } = item as AbiFunctionFragment;
                return (
              ...
              if (abi) {
                assert(abi?.type === "function", METHOD_NOT_FOUND);
                assert(abi?.name, METHOD_NOT_FOUND);
                ...
                multicall.push({
                  ...
                  selector(data: string) {
                    return ABICoder.decodeParameters(
                      (abi?.outputs || []) as AbiInput[],
                      data
                    )[0];
                  },
                });
              } else {
                query.reject(new Error(METHOD_NOT_FOUND));
              }
            } else {
              const abi: AbiFunctionFragment =
                (query.abi as AbiFunctionFragment) ||
      ...
      singleCallQueries.forEach((query) => {
        ...
        if (type === "getData") {
          const abi: AbiFunctionFragment =
            LSP8IdentifiableDigitalAssetContract.abi.find(
      ...
      const doSinglecall = async (singlecall: Multicall) => {
        ...
        const { web3 } = networkInfo;
        const { target, call } = singlecall;
        assert(web3, "Network incorrectly configured");
        try {
...
export type CallContractQueryOptions = {
  abi?: AbiFunctionFragment | readonly AbiItem[];
  address: `0x${string}`;
  method: string;
  args?: readonly unknown[];
  chainId: string;
...
export function queryCallContract<T>({
  ...
  let methodItem: AbiFunctionFragment =
    (Array.isArray(abi) ? undefined : (abi as AbiFunctionFragment)) ||
    (((Array.isArray(abi) ? (abi as AbiItem[]) : undefined) || defaultAbi).find(
      (item) => {
        if (item.type !== "function") {
          return false;
        }
...
export type VerifiableURI = {
  verification: {
    method: `0x${string}` | "keccak256(utf8)" | "keccak256(bytes)";
    data: `0x${string}`;
  };
  url: string;
};
