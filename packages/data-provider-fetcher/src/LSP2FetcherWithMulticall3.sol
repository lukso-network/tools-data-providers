...
// DEPLOYED and TESTED (thanks for mainnet deployment @b00ste)
// MAINNET: 0x51c602b249a5bc0ff437fbed4607448d8bc66dad
// TESTNET: 0x1690c1A6f5dc385d20139222567476F0A185f77a

import {IERC725Y} from "ERC725/implementations/contracts/interfaces/IERC725Y.sol";
import {LSP8IdentifiableDigitalAssetCore} from "lsp-smart-contracts/packages/lsp8-contracts/contracts/LSP8IdentifiableDigitalAssetCore.sol";
...
contract LSP2FetcherWithMulticall3 {
    ...
    // success = 0 => failed
    // success = 1 => success
    // success = 2 => success truncated
    // success = 3 => skipped (unknown success)
    struct Result4 {
        uint256 success;
        uint256 totalLength;
        bytes returnData;
    ...
    function aggregate4(
        ...
        for (uint256 i = 0; i < length; ) {
            ...
            bool success;
            (success, callResult) = calli.target.call(calli.callData);
            if (!calli.allowFailure) {
                require(success, "aggregate4: call failed");
            }
            if (success) {
                result.totalLength = callResult.length;
                result.success = 1; // 1 is success
                if (result.totalLength > limit) {
                    ...
                    result.success = 2; // truncated
                    unchecked {
                        limit -= result.returnData.length;
                    }
                    for (uint256 j = i + 1; j < length; j++) {
                        returnData[j].success = 3; // skipped
                    }
                    break;
                } else {
    ...
    function getDataForTokenIdSlice(
        ...
        bytes memory data = LSP8IdentifiableDigitalAssetCore(_contract)
